...
//  EnergyShape
//
//  Created by Sun on 2026/1/23.
//  Metal Shader 源码 - 嵌入式字符串加载
//
...
/// Shader 源码管理
/// 将 Metal 着色器代码以字符串形式嵌入，避免依赖 .metal 文件
enum ShaderSource {
    /// Metal Shader 源码
    ...
    struct EnergyUniforms {
        ...
        float noiseScale;        // 噪声缩放（影响流动图案密度）
        ...
        // 边框发光参数
        float glowFalloff;       // 发光衰减距离（像素单位）- 值越大光晕越宽越柔和
        float borderBandWidth;   // 边框带宽度（像素单位）- 边框带的实际宽度
        float borderSoftness;    // 边框柔和度 [0~1]：0=均匀亮度，1=渐变
        float innerGlowIntensity; // 内发光强度
        float innerGlowRadius;   // 内发光半径（像素单位）
        float outerGlowIntensity; // 外发光强度
        float outerGlowRadius;   // 外发光半径（像素单位）
    ...
    /// 改进版：使用 alpha 梯度检测边缘，保留 CGContext 栅格化时的抗锯齿信息
    kernel void jfaSeedInit(
        ...
        for (int dy = -1; dy <= 1; dy++) {
            for (int dx = -1; dx <= 1; dx++) {
                ...
                gradX += neighborMask * wx;
                gradY += neighborMask * wy;
            }
        }
        
        // 梯度强度表示边缘可能性
        float gradientMagnitude = length(float2(gradX, gradY));
        
        // 边缘判定：梯度强度超过阈值且像素本身有一定 alpha 值
        // 阈值 0.3 是经验值，能够捕获抗锯齿过渡区域
        bool isEdge = (gradientMagnitude > 0.3) && (mask > 0.01);
    ...
    kernel void jfaToSDF(
        ...
        // 有符号距离：内部为负，外部为正（标准 SDF 惯例，与解析 SDF 一致）
        float signedDist = (mask > 0.5) ? -dist : dist;
        
        // 归一化到 [0, 1]，0.5 表示边缘（内部 < 0.5，外部 > 0.5）
    ...
    fragment float4 fragmentEnergy(
        ...
        // 计算 SDF 纹理的 texel 尺寸
        float2 sdfTexelSize = float2(1.0 / sdfTexture.get_width(), 1.0 / sdfTexture.get_height());

        // 1. 采样 SDF - 使用高斯模糊多采样获得更平滑的距离值
        // 3x3 高斯核权重 (sigma ≈ 0.85)
        float sdf = 0.0;
        ...
        for (int dy = -1; dy <= 1; dy++) {
            for (int dx = -1; dx <= 1; dx++) {
                float2 offset = float2(float(dx), float(dy)) * sdfTexelSize;
                sdf += sdfTexture.sample(texSampler, uv + offset).r * weights[idx++];
            }
        }
        sdf /= totalWeight;

        // SDF 转换：sdf=0.5 表示边缘，<0.5 表示内部，>0.5 表示外部
        // signedDistPx: 负数表示内部，正数表示外部（标准 SDF 惯例）
        float distNorm = (sdf - 0.5) * 2.0;
        float signedDistPx = distNorm * uniforms.sdfMaxDist;
        
        // 2. 计算抗锯齿过渡宽度
        // 对于 JFA SDF，使用更大的抗锯齿带以补偿离散化误差
        // 基础宽度 = fwidth * 3.0，最小 1.5 像素
        float aaWidth = max(fwidth(signedDistPx) * 3.0, 1.5);
        
        // 3. 内部/外部距离（与 fragmentEnergyAnalytic 一致）
        float innerDist = max(0.0, -signedDistPx);
        float outerDist = max(0.0, signedDistPx);
        
        // 4. 检查外发光是否启用
        bool outerGlowEnabled = uniforms.outerGlowIntensity > 0.001 && uniforms.outerGlowRadius > 0.001;
        ...
        if (signedDistPx > 0.0) {
            ...
            if (!outerGlowEnabled) {
                ...
                // 外发光开启：超出外发光范围则不渲染
                if (outerDist > uniforms.outerGlowRadius) {
                    return float4(0.0);
                }
            }
        }
        ...
        if (signedDistPx <= 0.0) {
            float thickness = uniforms.borderBandWidth;
            ...
            if (thickness > 0.1) {
                ...
                if (innerDist < thickness) {
                    ...
                    borderGlow = exp(-distBeyondBorder / max(uniforms.glowFalloff, 1.0)) * uniforms.glowIntensity;
                }
            } else {
                // 纯发光衰减模式（原有行为）
                borderGlow = exp(-innerDist / max(uniforms.glowFalloff, 1.0)) * uniforms.glowIntensity;
            }
        }
        ...
        float innerGlow = 0.0;
        if (signedDistPx <= 0.0) {
            innerGlow = exp(-innerDist / max(uniforms.innerGlowRadius, 1.0)) * uniforms.innerGlowIntensity;
        }
        ...
        if (outerGlowEnabled && signedDistPx > 0.0) {
            // 使用更陡峭的衰减曲线
            float normalizedDist = outerDist / uniforms.outerGlowRadius;
        ...
        float edgeEnhanceRange = max(uniforms.borderBandWidth, uniforms.glowFalloff) * 3.0;
    ...
    fragment float4 fragmentEnergyNoSDF(
        ...
        float thickness = uniforms.borderBandWidth * 0.1;  // 归一化到 depth 空间
        float softness = uniforms.borderSoftness;
        float borderGlow = 0.0;
        ...
        if (thickness > 0.001 && depth > 0.0) {
            ...
            if (innerDist < thickness) {
                ...
                float distBeyondBorder = innerDist - thickness;
                borderGlow = exp(-distBeyondBorder * 10.0 / max(uniforms.glowFalloff, 0.1)) * uniforms.glowIntensity;
            }
        } else {
            // 纯发光衰减模式（原有行为）
            borderGlow = exp(-absDepth * 10.0 / max(uniforms.glowFalloff, 0.1)) * uniforms.glowIntensity;
        }
        borderGlow = max(borderGlow, edgeStrength * 0.8 * uniforms.glowIntensity);
        
        // 4. 内发光（从边缘向内部扩散）
        float innerDist = max(0.0, depth);
        float innerGlow = exp(-innerDist * 5.0 / max(uniforms.innerGlowRadius, 0.01)) * uniforms.innerGlowIntensity;
        
        // 5. 外发光（从边缘向外部扩散）
        float outerDist = max(0.0, -depth);
        float outerGlow = exp(-outerDist * 5.0 / max(uniforms.outerGlowRadius, 0.01)) * uniforms.outerGlowIntensity;
        ...
        // 使用分形噪声
        float noise = fbmNoise(uv * uniforms.noiseScale, uniforms.time * 0.15, uniforms.noiseOctaves, 2.0);
    ...
    /// 从源码字符串创建 Metal Library
    /// - Parameter device: Metal 设备
    /// - Returns: 编译后的 MTLLibrary
    /// - Throws: 编译错误
    static func makeLibrary(device: MTLDevice) throws -> MTLLibrary {
        do {
            return try device.makeLibrary(source: source, options: nil)
        } catch {
            throw EnergyShapeError.shaderCompilationFailed("Shader 编译失败: \(error.localizedDescription)")
        }
    }
}
