...
final class GPUSDFComputer: SDFProvider {
    ...
    private func setupPipelines() throws {
        ...
        guard let library = device.makeDefaultLibrary() else {
            // 如果默认库不存在，从源码编译
            guard let library = try? loadLibraryFromSource() else {
                throw SDFError.computeFailed("无法加载 Metal 库")
            }
    ...
    private func loadLibraryFromSource() throws -> MTLLibrary {
        // 从 Bundle 加载 Shader 源码
        let bundle = Bundle(for: GPUSDFComputer.self)
        ...
        throw SDFError.computeFailed("找不到 Shader 源文件")
    }
    ...
    func prepareSDF(
        ...
        let task = DispatchWorkItem { [weak self] in
            ...
            do {
                // 1. 栅格化路径为 Mask
                let maskTexture = try self.rasterizePath(
                    path,
                    width: textureWidth,
                ...
                // 2. 使用 JFA 计算 SDF
                let sdfTexture = try self.computeSDFWithJFA(from: maskTexture)
    ...
    private func computeSDFWithJFA(from maskTexture: MTLTexture) throws -> MTLTexture {
        ...
        // 确保 Pipeline 存在
        guard let seedPipeline = seedPipelineState,
        ...
        guard let ping = device.makeTexture(descriptor: workDescriptor),
