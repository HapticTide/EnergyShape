...
final class EnergyMaskCache {
    ...
    func updatePath(
        ...
        generateQueue.async { [weak self] in
            ...
            do {
                // 计算纹理尺寸
                let textureWidth = Int(size.width * scale)
                let textureHeight = Int(size.height * scale)
                ...
                // 生成 Mask 纹理
                let maskTexture = try self.generateMaskTexture(
                    from: path,
                    width: textureWidth,
                ...
                if sdfEnabled {
                    ...
                    let sdfWidth = max(1, Int(Float(textureWidth) * sdfScale))
                    let sdfHeight = max(1, Int(Float(textureHeight) * sdfScale))
                    
                    sdfTexture = try self.generateSDFTexture(
                        from: maskTexture,
    ...
    func updateColorLUT(_ colorStops: [ColorStop]) {
        generateQueue.async { [weak self] in
            ...
            if let texture = self.generateLUTTexture(from: colorStops) {
                self.lock.lock()
    ...
    private func computeSDF(from mask: [UInt8], width: Int, height: Int) -> [UInt8] {
        let inf: Int32 = Int32(width + height)
    ...
    private func downsample(_ data: [UInt8], from source: (Int, Int), to target: (Int, Int)) -> [UInt8] {
        ...
        for y in 0..<target.1 {
            for x in 0..<target.0 {
                let srcX = min(source.0 - 1, Int(Float(x) * scaleX))
                let srcY = min(source.1 - 1, Int(Float(y) * scaleY))
                result[y * target.0 + x] = data[srcY * source.0 + srcX]
            }
        }
